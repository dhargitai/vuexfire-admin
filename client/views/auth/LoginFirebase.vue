                            ...
                            <div class="padded-box">
                                <div class="card">
                                    ...
                                    <transition name="scaleFade" mode="out-in" appear>
                                        <div v-if="item.type === 'form' && !authUI.loginBtnDisabled" class="card-content">
                                            <user-pass-form></user-pass-form>
                                        </div>
                                    </transition>
                                    <div v-if="item.type === 'button' && !authUI.loginBtnDisabled" class="card-footer">
                                        <a :disabled="authUI.loginBtnDisabled" @click="signIn(item.icon)" class="card-footer-item" v-bind:class="{ 'is-loading': authUI.pending }" id="quickstart-sign-in">
                                            <i :class="'fa fa-pull-left fa-' + item.icon"></i> Sign in
                                        </a>
                                    </div>
                                </div>
                            </div>
...
.loggedInAnimation {
    margin-bottom: 1rem; // padding:1rem 2rem;
